...
| `kaplan_meier` | Kaplan-Meier survival | `crates/bl-runtime/src/bio_plots/clinical.rs` | SVG/terminal/HTML `Str`; spec `Record` | Yes, `biolang.plot.spec/v1` | Tied event/censor risk sets, product-limit survival, Greenwood log-scale confidence intervals, risk tables, p-value labels and safe custom palettes. Geometry matches independent `survival::survfit` output. |
