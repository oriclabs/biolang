(function() {
  ...
  function checkExtension(name) {
    ...
    if (desc) return { reason: desc, hint: "BioPeek is for bioinformatics data files (FASTA, FASTQ, VCF, BED, GFF, SAM, CSV, TSV).\nThis appears to be a " + desc.toLowerCase() + "." };
  ...
  function checkMagicBytes(bytes, name) {
    ...
    if (nonPrint > check * 0.3) {
      return { ok: false, reason: "Binary file", hint: "This appears to be a binary file. BioPeek supports text-based formats only." };
    }
  ...
  function renderChromStrip(f) {
    ...
    entries.forEach(function(e) {
      ...
      tag.addEventListener("click", function() {
        // Quick filter to this chromosome (toggle behavior)
        ...
        // Update active state on all chrom tags
        strip.querySelectorAll(".vw-chrom-tag").forEach(function(t) {
  ...
  function drawTitle(ctx, f, isDark, canvasW) {
    ...
    ctx.fillText("BioPeek — " + f.name, 20, 28);
  ...
  function screenshotCanvas(f, isDark) {
    ...
    if (currentView === "stats" && f.parsed.stats) {
      ...
      ctx.fillText("BioPeek — lang.bio", 800 - 120, canvasH - 8);
      ...
      ctx.fillStyle = isDark ? "#1e293b" : "#e2e8f0";
      ctx.font = "9px system-ui";
      ctx.fillText("BioPeek — lang.bio", canvasW - 120, canvasH - 8);
    }
  ...
  function generateShareLink() {
    ...
    overlay.innerHTML =
      ...
      '<p style="font-size:11px;color:var(--vw-text-dim);margin:0 0 8px">Copy this URL — anyone who opens it will see this file in BioPeek.</p>' +
  ...
  if (colorToggleBtn) {
    if (!seqColorEnabled) colorToggleBtn.classList.add("vw-tbtn-off");
    colorToggleBtn.addEventListener("click", function() {
      ...
      colorToggleBtn.classList.toggle("vw-tbtn-off", !seqColorEnabled);
      renderView();
    });
  }
  ...
  function exportAsHtml() {
    ...
    html += '</tbody></table>';
    html += '<p style="color:#565f89;font-size:10px;margin-top:16px">Generated by BioPeek</p>';
    html += '</body></html>';
