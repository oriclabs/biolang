...
browser.runtime.onInstalled.addListener(() => {
  browser.alarms.create(ALARM_NAME, { periodInMinutes: ALARM_INTERVAL_MINUTES });
...
async function runCheck() {
  const data = await browser.storage.local.get(["biokhoj_watchlist", "biokhoj_papers", "biokhoj_settings"]);
  ...
  if (newPapers.length > 0) {
    ...
    if (highPriority.length > 0) {
      ...
      browser.notifications.create("biokhoj-new-" + now, {
        type: "basic",
...
async function fetchCitationsForReadingList() {
  try {
    const data = await browser.storage.local.get(["biokhoj_reading_list", "biokhoj_citations"]);
    ...
    await browser.storage.local.set({ biokhoj_citations: citations });
  } catch (e) {
...
browser.contextMenus.onClicked.addListener(async (info, tab) => {
  ...
  if (info.menuItemId === "biokhoj-lookup") {
    ...
    if (tab && tab.id) browser.sidePanel.open({ tabId: tab.id }).catch(() => {});
    browser.storage.session.set({ biokhoj_lookup: { term: entity.name, type: entity.type, ts: Date.now() } });
    return;
  }
    ...
    if (pmid) {
      try {
        ...
        if (papers.length > 0) {
          ...
          const data = await browser.storage.local.get("biokhoj_papers");
          ...
          if (!existing.some(p => p.pmid === pmid)) {
            ...
            await browser.storage.local.set({ biokhoj_papers: existing.slice(0, MAX_PAPERS) });
          }
          browser.notifications.create("biokhoj-saved-" + Date.now(), {
  ...
  if (tab && tab.id) {
    browser.sidePanel.open({ tabId: tab.id }).catch(() => {});
  }

  const data = await browser.storage.local.get("biokhoj_watchlist");
  const watchlist = data.biokhoj_watchlist || [];

  // Skip duplicates (case-insensitive, name only)
  if (watchlist.some(w => w.name.toLowerCase() === entity.name.toLowerCase())) {
    browser.notifications.create("biokhoj-dup-" + Date.now(), {
      type: "basic",
      iconUrl: "icons/icon128.png",
      title: "BioKhoj",
  ...
  await browser.storage.local.set({ biokhoj_watchlist: watchlist });

  browser.notifications.create("biokhoj-add-" + Date.now(), {
    type: "basic",
    iconUrl: "icons/icon128.png",
    title: "BioKhoj",
...
function notifySidebarRefresh() {
  browser.runtime.sendMessage({ type: "watchlist-updated" }).catch(() => {});
}
...
browser.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.type === "clear-papers") {
    ...
    browser.storage.local.get("biokhoj_papers", (data) => {
      ...
      browser.storage.local.set({ biokhoj_papers: papers }, () => {
  ...
  if (msg.type === "get-watchlist") {
    browser.storage.local.get("biokhoj_watchlist", (data) => {
  ...
  if (msg.type === "add-watch") {
    browser.storage.local.get("biokhoj_watchlist", (data) => {
      ...
      browser.storage.local.set({ biokhoj_watchlist: watchlist }, () => {
        notifySidebarRefresh();
  ...
  if (msg.type === "get-citations") {
    browser.storage.local.get("biokhoj_citations", (data) => {
  ...
  if (msg.type === "open-pwa") {
    browser.tabs.create({ url: "https://lang.bio/biokhoj" });
    sendResponse({ ok: true });
    return true;
  }
...
async function fetchTrendingBioRxiv(force) {
  ...
  // Also check browser.storage cache
  if (!force) {
    try {
      const stored = await browser.storage.local.get("biokhoj_trending");
  ...
  try {
    ...
    await browser.storage.local.set({ biokhoj_trending: { papers, ts: now } });
    return papers;
  } catch (e) {
...
async function fetchAllTrending(force) {
  ...
  if (!force) {
    try {
      const stored = await browser.storage.local.get("biokhoj_all_trending");
  ...
  await browser.storage.local.set({ biokhoj_all_trending: { results, ts: now } });
  return results;
}
