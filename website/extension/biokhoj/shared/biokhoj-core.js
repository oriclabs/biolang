...
(function() {
   ...
   */
  async function searchPubMed(query, maxResults = 20, minDate = null) {
    try {
  ...
  function _coMentionKey(entityA, entityB) {
    const a = (entityA.name || entityA.id || "").toLowerCase();
    const b = (entityB.name || entityB.id || "").toLowerCase();
    return a < b ? `${a}::${b}` : `${b}::${a}`;
  }
    ...
    for (const paper of papers) {
      ...
      for (const entity of watchedEntities) {
        ...
        if (name && text.includes(name)) {
          matched.add(entity.name || entity.id);
        }
      }
