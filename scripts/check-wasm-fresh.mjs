 ...
 *
 * `website/wasm/bl_wasm_bg.wasm` is a build artifact that lives in the
 ...
 *
 * check-generated already compares the two copies of the module against each
 * other. That catches a rebuild that updated only one of them, but it cannot
 * catch the case where both are equally out of date, which is the common one.
 * Two identical stale files pass it.
 *
 ...
 *            the same route website/tests/write_wasm_builtins.mjs takes
 *
 ...
 *
 * Either way the fix is one command, which rebuilds, copies to the workbench
...
const module_ = path.join(websiteRoot, "wasm", "bl_wasm_bg.wasm");
 ...
 *
 * Regenerating the catalog is the honest way to ask: builtins.json is
 * gitignored, so a stale copy from an earlier build must not be trusted.
 */
async function builtinsInModule() {
  await execFileAsync("node", ["tests/write_wasm_builtins.mjs"], {
    cwd: websiteRoot,
    maxBuffer: 8 * 1024 * 1024,
  });
  const catalog = path.join(websiteRoot, "wasm", "builtins.json");
  return new Set(JSON.parse(readFileSync(catalog, "utf8")).builtins ?? []);
}
...
function fingerprintDrift() {
  const recorded = path.join(websiteRoot, "wasm-build-fingerprint.json");
...
if (missing.length || extra.length || drift) {
  ...
  console.error("       (rebuilds, copies to the workbench, re-records the fingerprint)\n");
  process.exit(1);
}
