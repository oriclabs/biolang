...
//!
//! Written solely from the published descriptions. No third-party
//! implementation source was used; this crate remains MIT.
//!
...
fn fit_nb_offset_cox_reid(
    ...
    let fit_at = |log_theta: f64| -> (f64, f64) {
        ...
        for &(cell, count) in counts {
            let mu = (cell_totals[cell] * scale).max(1e-12);
            let sum = theta + mu;
            log_likelihood += crate::stats_ops::ln_gamma(count + theta)
                - crate::stats_ops::ln_gamma(theta)
                - crate::stats_ops::ln_gamma(count + 1.0)
                + count * (mu.ln() - sum.ln());
        }
...
fn density_weighted_sample(
    ...
    let x: Vec<f64> = candidates.iter().map(|gene| log_means[*gene]).collect();
    ...
    let mut last = 0usize;
    for index in 0..n {
        while first < index && x[index] - x[first] > 4.0 * bandwidth {
            first += 1;
        }
        if last < index {
            last = index;
        }
        while last + 1 < n && x[last + 1] - x[index] <= 4.0 * bandwidth {
            last += 1;
        }
        let density_sum = (first..=last)
            .map(|other| {
                let z = (x[other] - x[index]) / bandwidth;
                (-0.5 * z * z).exp()
            })
            .sum::<f64>();
        weighted_candidates.push((1.0 / density_sum.max(f64::MIN_POSITIVE), candidates[index]));
    }

    // R's documented weighted-without-replacement contract draws sequentially
    // from the remaining mass. Sorting by descending probability is
    // numerically stable and is observable through the public `sample` API.
    weighted_candidates
        .sort_by(|left, right| right.0.total_cmp(&left.0).then(left.1.cmp(&right.1)));
    let mut total_weight: f64 = weighted_candidates.iter().map(|item| item.0).sum();
    ...
    for _ in 0..wanted {
        ...
        for (slot, &(weight, _)) in weighted_candidates.iter().enumerate() {
            cumulative += weight;
...
pub fn sctransform(data: &GeneColumns, options: &SctOptions) -> SctResult {
    ...
    for gene in 0..n_genes {
        ...
        poisson_like[gene] = mean < 0.001 || variance <= mean;
    ...
    // the crowded middle.
    let mut by_expression: Vec<usize> = modelled
        .iter()
        .copied()
        .filter(|&gene| data.detected(gene) >= options.min_cells)
        .collect();
    by_expression.sort_by(|&a, &b| log_means[a].total_cmp(&log_means[b]));
    let fit_genes = density_weighted_sample(
        &by_expression,
        &log_means,
...
mod tests {
    ...
    fn density_weighted_sampling_represents_sparse_expression_tails() {
        ...
        assert_eq!(selected, selected_again, "the seeded sample changed");
