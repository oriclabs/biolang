...
fn builtin_parse_gtf(args: Vec<Value>) -> Result<Value> {
    ...
    for line in text.lines() {
        ...
        let get_or = |k1: &str, k2: &str| -> String {
            if let Some(v) = attrs.get(k1) {
                v.clone()
            } else {
                attrs.get(k2).cloned().unwrap_or_default()
            }
        };
...
fn builtin_gene_bodies(args: Vec<Value>) -> Result<Value> {
    ...
    for row in &t.rows {
        ...
        let name = ci_gene_name
            .map(|i| str_val(&row[i]).to_string())
            .unwrap_or_default();
    ...
    let out_cols = vec!["gene_id", "gene_name", "chrom", "start", "end", "strand"]
        ...
        .map(|s| s.to_string())
        .collect();
    let rows: Vec<Vec<Value>> = gene_ids
        .iter()
        .map(|gid| {
            let g = &genes[gid];
            vec![
...
fn builtin_promoters(args: Vec<Value>) -> Result<Value> {
    ...
    let upstream = if args.len() > 1 {
        to_i64(&args[1])
    } else {
        2000
    };
    let downstream = if args.len() > 2 {
        to_i64(&args[2])
    } else {
        200
    };
...
fn builtin_annotate_peaks(args: Vec<Value>) -> Result<Value> {
    ...
    let genes: Vec<Gene> = gtf
        ...
        .filter_map(|row| {
            ...
            let gene_name = gc_gene_name
                .map(|i| str_val(&row[i]).to_string())
                .unwrap_or_default();
            let strand = gc_strand
                .map(|i| str_val(&row[i]).to_string())
                .unwrap_or("+".to_string());
...
fn builtin_gene_id_map(args: Vec<Value>) -> Result<Value> {
    ...
    let rows: Vec<Vec<Value>> = ids
        .iter()
        .map(|gid| vec![Value::Str(gid.clone()), Value::Str(seen[gid].clone())])
        .collect();
