...
fn test_allele_specific_cn_balanced() {
    ...
    if let Value::Table(t) = result {
        let total_cn = match t.rows[0][0] {
            Value::Int(n) => n,
            _ => panic!(),
        };
        let major_cn = match t.rows[0][1] {
            Value::Int(n) => n,
            _ => panic!(),
        };
        let minor_cn = match t.rows[0][2] {
            Value::Int(n) => n,
            _ => panic!(),
        };
...
fn test_insulation_score_shape() {
    ...
    let mat: Vec<Vec<f64>> = (0..n)
        .map(|i| (0..n).map(|j| if i == j { 10.0 } else { 1.0 }).collect())
        .collect();
    let input = make_square_table(&mat);
...
fn test_tad_boundaries_detects_minimum() {
    ...
    let scores =
        Value::List((vec![Value::Float(1.0), Value::Float(-0.5), Value::Float(1.0)]).into());
...
fn test_distance_decay_shape() {
    ...
    if let Value::Table(t) = result {
        ...
        let d0 = match t.rows[0][1] {
            Value::Float(f) => f,
            _ => panic!(),
        };
        assert!(
            (d0 - 10.0).abs() < 1e-6,
            "d=0 mean should be 10.0, got {d0}"
        );
    } else {
        panic!("expected Table");
    }
...
fn test_expected_contacts_shape() {
    ...
    if let Value::Table(t) = result {
        ...
        let diag = match t.rows[0][0] {
            Value::Float(f) => f,
            _ => panic!(),
        };
        assert!(
            (diag - 10.0).abs() < 1e-6,
            "diagonal should be 10.0, got {diag}"
        );
    } else {
        panic!("expected Table");
    }
