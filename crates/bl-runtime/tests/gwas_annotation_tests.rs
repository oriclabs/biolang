...
fn manhattan_data_cumulative_positions() {
    ...
    let cum: Vec<i64> = get_col(&t, "cumulative_pos")
        .iter()
        .map(|v| match v {
            Value::Int(n) => *n,
            _ => -1,
    ...
    assert!(
        as_float(&nlp[1]) > as_float(&nlp[0]),
        "lower p → higher -log10p"
    );
}
...
fn clump_removes_nearby_snps() {
    ...
    assert_eq!(
        t.rows.len(),
        2,
        "second SNP within 250kb should be clumped out"
    );
}
...
fn lambda_gc_near_one_for_uniform() {
    ...
    assert!(
        (lam - 1.0).abs() < 0.15,
        "lambda_gc for uniform pvals should be ~1, got {lam}"
    );
}
