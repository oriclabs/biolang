...
fn stats_registry_complete() {
    ...
    for (name, _) in &list {
        assert!(
            ...
            "{name} missing from is_statistics_builtin"
        );
    }
...
fn bootstrap_ci_contains_true_mean() {
    ...
    let result = call_statistics_builtin(
        "bootstrap_ci",
        vec![vals, Value::Int(500), Value::Float(0.95)],
    )
    .unwrap();
...
fn genomic_inflation_inflated() {
    ...
    assert!(
        lambda > 1.5,
        "expected lambda > 1.5 for small ps, got {lambda}"
    );
}
...
fn pearson_correlation_zero_variance() {
    ...
    assert!(
        r.abs() < 1e-10,
        "expected r=0.0 for zero-variance x, got {r}"
    );
}
...
fn delta_ct_scalar() {
    let result =
        call_qpcr_builtin("delta_ct", vec![Value::Float(28.5), Value::Float(25.0)]).unwrap();
...
fn delta_delta_ct_no_change() {
    ...
    let result =
        call_qpcr_builtin("delta_delta_ct", vec![Value::Float(3.0), Value::Float(3.0)]).unwrap();
    let fc = extract_float(&result);
    assert!(
        (fc - 1.0).abs() < 1e-10,
        "expected fold change=1.0, got {fc}"
    );
}
...
fn delta_delta_ct_two_fold_up() {
    ...
    let result =
        call_qpcr_builtin("delta_delta_ct", vec![Value::Float(2.0), Value::Float(3.0)]).unwrap();
    let fc = extract_float(&result);
    assert!(
        (fc - 2.0).abs() < 1e-10,
        "expected fold change=2.0, got {fc}"
    );
}
...
fn pcr_efficiency_ideal() {
    ...
    assert!(
        (eff - 1.0).abs() < 0.05,
        "expected efficiency≈1.0, got {eff}"
    );
}
...
fn genorm_stability_most_stable_first() {
    ...
    match &result {
        Value::Table(t) => {
            ...
            assert!(
                first_m <= last_m,
                "expected ascending M-scores, first={first_m} last={last_m}"
            );
        }
        _ => panic!("expected Table"),
    }
...
fn unknown_statistics_builtin() {
    ...
    assert!(
        ...
        "error should mention builtin name"
    );
}
