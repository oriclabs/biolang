...
fn test_score_matrix_blosum62() {
    let result =
        call_alignment_builtin("score_matrix", vec![Value::Str("blosum62".into())]).unwrap();
...
fn test_msa_with_divergent_sequences() {
    ...
    assert!(
        ...
        "aligned sequences must be equal length: {:?}",
        lens
    );
}
...
fn test_distance_matrix_hamming_model() {
    ...
    let result = call_alignment_builtin(
        "distance_matrix",
        vec![
            Value::List((vec![Value::Str("AATT".into()), Value::Str("AAAA".into())]).into()),
            Value::Record((opts).into()),
        ],
    )
    .unwrap();
...
fn test_distance_matrix_jc_model() {
    ...
    let result = call_alignment_builtin(
        "distance_matrix",
        vec![
            Value::List((vec![Value::Str("AACC".into()), Value::Str("AATT".into())]).into()),
            Value::Record((opts).into()),
        ],
    )
    .unwrap();
...
fn test_distance_matrix_unknown_model() {
    ...
    let result = call_alignment_builtin(
        "distance_matrix",
        vec![
            Value::List((vec![Value::Str("ACGT".into()), Value::Str("ACGA".into())]).into()),
            Value::Record((opts).into()),
        ],
    );
    assert!(result.is_err());
    ...
    if let Value::List(scores) = result {
        ...
        for s in scores.iter() {
            if let Value::Float(f) = s {
                assert!(
                    *f > 0.0 && *f <= 1.0,
                    "partial conservation should be in (0, 1], got {f}"
                );
            }
        }
    } else {
...
fn test_msa_to_distance_matrix_to_conservation_pipeline() {
    ...
    // Step 1: MSA
    let msa = call_alignment_builtin("msa", vec![Value::List((seqs).into())]).unwrap();
