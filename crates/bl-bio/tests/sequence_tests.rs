...
fn test_find_motif() {
    ...
    assert_eq!(
        result,
        Value::List((vec![Value::Int(0), Value::Int(3), Value::Int(6)]).into())
    );
}
...
fn test_subseq() {
    ...
    .unwrap();
    assert_eq!(result, Value::DNA(BioSequence { data: "GAT".into() }));
}
...
fn test_to_interval_record() {
    ...
    let result = call_bio_builtin("to_interval", vec![Value::Record((fields).into())]).unwrap();
    assert_eq!(
        result,
...
fn test_parse_info_key() {
    ...
    let result =
        call_bio_builtin("parse_info", vec![info.clone(), Value::Str("DP".into())]).unwrap();
    assert_eq!(result, Value::Int(100));

    let result2 =
        call_bio_builtin("parse_info", vec![info.clone(), Value::Str("DB".into())]).unwrap();
...
fn test_parse_info_from_record() {
    ...
    let result = call_bio_builtin(
        "parse_info",
        vec![Value::Record((fields).into()), Value::Str("DP".into())],
    )
    .unwrap();
...
fn test_parse_attr_gtf() {
    let attr = Value::Str(
        r#"gene_id "ENSG00000141510"; gene_name "TP53"; biotype "protein_coding""#.into(),
    );
    let result = call_bio_builtin("parse_attr", vec![attr]).unwrap();
    if let Value::Record(fields) = result {
...
fn test_parse_qual() {
    ...
    assert_eq!(
        result,
        Value::List((vec![Value::Int(0), Value::Int(40), Value::Int(93)]).into())
    );
}
