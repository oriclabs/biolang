...
fn notebook_page(state: &ServerState) -> String {
    ...
    let figure_runtime = include_str!("../assets/figure-fallback.js");
    NOTEBOOK_PAGE
        .replace("{title}", &title)
        .replace("{token}", &token)
...
.cell-figure-controls{display:flex;justify-content:flex-end;gap:.35rem;margin-bottom:.35rem}.cell-figure-toggle{border:1px solid #cbd5e1;border-radius:4px;background:#f8fafc;color:#334155;padding:.2rem .5rem;font:11px system-ui,sans-serif;cursor:pointer}.cell-figure-toggle:disabled{display:none}.cell-figure-canvas{display:block}.cell-figure-canvas[hidden],.cell-figure svg[hidden]{display:none}
</style></head><body>
...
mod tests {
    ...
    fn generated_page_contains_no_token_placeholder() {
        ...
        assert!(page.contains("cell-figure-controls"));
