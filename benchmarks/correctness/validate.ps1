...
def compare(a, b, path='', tol=1e-6):
    ...
    return []
# PowerShell's > redirection writes UTF-8 with a BOM; utf-8-sig strips one if
# present and is harmless if not, so this reads output from either runner.
...
function Run-Comparison($label, $refCmd, $refScript, $blScript, $task) {
    ...
    # Anything a native command writes to stderr becomes a NativeCommandError
    ...
    # Compare. Same stderr rule as above: the comparator prints its diff on
    # stderr, which would terminate this function before it could report one.
    $prev = $ErrorActionPreference
    $ErrorActionPreference = "Continue"
    try {
...
Write-Evidence -Slug "synthetic" -Suite "Synthetic-data correctness" -DataDesc "generated by benchmarks/generate_data.py" `
